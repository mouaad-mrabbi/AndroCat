...
export default function FileManager() {
  ...
  const [selectedOption, setSelectedOption] = useState("apks");
  const [titleFile, setTitleFile] = useState("");
  const [randomText, setRandomText] = useState<string>("");
  const [randomNum, setRandomNum] = useState<number>(0);

  const handleTitleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleOptionChange = (e: any) => {
    setSelectedOption(e.target.value); // تحديث الخيار المختار
  };
  ...
  const handleUpload = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Make sure to clean the titleFile properly
      const cleanTitleFile = slugify(titleFile.replace(/[;:\/=[\]{}.]/g, "-"), {
        replacement: "-",
        lower: true,
        strict: true,
        remove: /[^\w\s-]/g,
      ...
      // Build the fileName properly
      const fileName = `${selectedOption}/${randomText}/${cleanTitleFile}-${randomNum}`;
  ...
  return (
    <div className="max-w-2xl mx-auto mt-24 p-6 bg-white rounded-lg shadow-lg">
      ...
      {/* إضافة input لعرض وتغيير النص العشوائي */}
      <div className="mb-6 text-black">
      ...
      <div className="flex items-center gap-4 text-black">
        <input
          type="text"
          value={randomNum}
          readOnly // المستخدم لا يستطيع تغييره من الكيبورد
